(* The opam base images and per-compiler images.

   Each linux/windows distro gets a base image (opam + an initialised
   opam-repository) and then a compiler image per OCaml version built on top of
   it, plus (for the master distro) the opam package archive image. *)
...
module Make (OCurrent : S.OCURRENT) = struct
  ...
  module Arch = struct
    ...
    let pipeline ~ocluster ~repos ?(windows_version = Windows_map.empty) ~distro arch =
      ...
      let archive_image =
        if distro = Distro_util.master_distro && arch = `X86_64 then
          let push_target =
  ...
  let pipeline ~ocluster ~repos ?windows_version distro gen_tags =
    ...
    let (multiarch_images, ocaml_images) =
      Distro_util.all_switches ocaml_images |> List.filter_map @@ begin fun switch ->
        match List.filter_map (Switch_map.find_opt switch) ocaml_images with
        | [] -> None
        | images ->
           let full_tag = Tag.v distro ~switch in
           let (multiarch_images, tags) = gen_tags images full_tag switch (Distro_util.aliases_of distro) in
      ...
      let gen_tags _images full_tag switch distro_aliases =
        let tags =
          ...
          if switch <> Ocaml_version.Releases.latest then tags
            ...
            if distro <> Distro_util.master_distro then tags
  ...
  (* Builds images for all supported distribution, compiler version and architecture combinations. *)
