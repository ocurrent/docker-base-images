...
let git_repositories () = Git_repositories.get ~schedule:weekly
...
let aliases_of =
  let latest =
    Distro.distros
    |> List.map (fun d ->
           ((Distro.resolve_alias d : Distro.distro :> Distro.t), d))
  in
  fun d ->
    List.filter_map
      (fun (d2, alias) -> if d = d2 && d <> alias then Some alias else None)
      latest
...
let maybe_add_beta (run : 'a run) switch =
  if Ocaml_version.Releases.is_dev switch then
    run
      "opam repo add beta git+https://github.com/ocaml/ocaml-beta-repository \
       --set-default"
  else Dockerfile.empty

let maybe_add_multicore (run : 'a run) switch =
  if Ocaml_version.Configure_options.is_multicore switch then
    run
      "opam repo add multicore \
       git+https://github.com/ocaml-multicore/multicore-opam --set-default"
  else Dockerfile.empty
...
let maybe_install_secondary_compiler (run : 'a run) os_family switch =
  ...
  if
    Ocaml_version.compare switch dune_min_native_support < 0
    && os_family <> `Windows
  then run "opam install -y ocaml-secondary-compiler"
  else Dockerfile.empty
...
let install_package_archive opam_image =
  ...
  @@ from "alpine:latest"
  @@ copy ~chown:"0:0" ~from:"archive"
       ~src:[ "/home/opam/opam-repository/cache" ]
       ~dst:"/cache" ()
...
let install_compiler_df ~distro ~arch ~switch ?windows_port opam_image =
  let switch_name =
    Ocaml_version.(with_just_major_and_minor switch |> to_string)
  in
...
let maybe_add_overlay distro hash =
  ...
  | `Windows (`Msvc, _) ->
      Dockerfile.run
        "opam repo add ocurrent-overlay \
         git+https://github.com/ocurrent/opam-repository-mingw#%s \
         --set-default"
        hash
  | _ -> Dockerfile.empty
...
module Make (OCurrent : S.OCURRENT) = struct
  ...
  module Arch = struct
    ...
    let install_opam ~arch ~ocluster ~distro ~repos ~push_target =
      ...
      let dockerfile =
        ...
        `Contents
          (let opam =
             snd
             @@ Dockerfile_opam.gen_opam2_distro
                  ~win10_revision:Conf.win10_revision ~arch
                  ~clone_opam_repo:false ~opam_hashes distro
           in
           let open Dockerfile in
           string_of_t
             (opam
             ...
             match os_family with
             | `Linux ->
                 copy ~link:true ~chown:"opam:opam" ~src:[ "." ]
                   ~dst:"/home/opam/opam-repository" ()
                 @@ run "opam-sandbox-disable"
                 @@ run
                      "opam init -k local -a /home/opam/opam-repository --bare"
                 @@ run "rm -rf .opam/repo/default/.git"
                 @@ copy ~link:true ~src:[ "Dockerfile" ]
                      ~dst:"/Dockerfile.opam" ()
             | `Windows ->
                 let opam_repo =
                   Windows.Cygwin.default.root ^ {|\home\opam\opam-repository|}
                 in
                 let opam_root = {|C:\opam\.opam|} in
                 copy ~src:[ "." ] ~dst:opam_repo ()
                 @@ env [ ("OPAMROOT", opam_root) ]
                 @@ run
                      "opam init -k local -a \"%s\" --bare --disable-sandboxing"
                      opam_repo
                 @@ maybe_add_overlay distro
                      (Current_git.Commit_id.hash opam_overlays)
                 @@ Windows.Cygwin.run_sh
                      "rm -rf /cygdrive/c/opam/.opam/repo/default/.git"
      ...
      let opam_repository =
        match os_family with
        | `Windows -> opam_repository_mingw_sunset
        | _ -> opam_repository_master
      in
      OCluster.Raw.build_and_push ocluster ~src:[ opam_repository ] dockerfile
        ~cache_hint ~options ~push_target
        ~pool:(Conf.pool_name distro arch)

    let install_compiler ~distro ~arch ~ocluster ~switch ~push_target
        ?windows_port base =
      let arch_name = Ocaml_version.string_of_arch arch in
      ...
      let dockerfile =
        `Contents
          (install_compiler_df ~distro ~arch ~switch ?windows_port base
      ...
      let cache_hint =
        Printf.sprintf "%s-%s-%s"
          (Ocaml_version.to_string switch)
          arch_name base
      in
      OCluster.Raw.build_and_push ocluster ~src:[] dockerfile ~cache_hint
        ~options ~push_target
        ~pool:(Conf.pool_name distro arch)
    ...
    let collect_archive ~distro ~ocluster ~push_target base =
      ...
      let> base in
      let dockerfile =
        `Contents (install_package_archive base |> Dockerfile.string_of_t)
      in
      let options =
        {
          Cluster_api.Docker.Spec.defaults with
    ...
    let pipeline ~ocluster ~repos ~distro arch =
      ...
      let opam_image =
        let push_target =
          Tag.v distro ~arch |> Cluster_api.Docker.Image_id.of_string |> or_die
        in
      ...
      let compiler_images =
        ...
        |> List.map @@ fun switch ->
           ...
           in
           let windows_port =
             match distro with `Windows (port, _) -> Some port | _ -> None
           in
           let repo_id =
             install_compiler ~distro ~arch ~ocluster ~switch ~push_target
               ?windows_port opam_image
           in
  ...
  let all_switches arches =
    ...
    arches
    |> ListLabels.fold_left ~init:Switch_set.empty ~f:(fun acc map ->
  ...
  let pipeline ~ocluster repos distro gen_tags =
    let opam_images, ocaml_images, archive_image =
      let arch_results =
        ...
        List.map (Arch.pipeline ~ocluster ~repos ~distro) arches
      in
      List.fold_left
        (fun (aa, ba, ca) (a, b, c) ->
    ...
    let multiarch_images, ocaml_images =
      ...
      |> ( List.filter_map @@ fun switch ->
           ...
           | images ->
               ...
               let multiarch_images, tags =
                 gen_tags images full_tag switch (aliases_of distro)
               in
               let pushes =
                 List.map
                   (fun tag ->
                     Docker.push_manifest ?auth:Conf.auth ~tag images
    ...
    let archive_images =
      ...
      | Some image ->
          [
            ( "archive",
              Docker.push_manifest ?auth:Conf.auth ~tag:(Tag.archive ())
                [ image ]
              |> Current.ignore_value );
          ]
    in
    let pipeline =
      Current.all_labelled
        (( "base",
           Docker.push_manifest ?auth:Conf.auth ~tag:(Tag.v distro) opam_images
           |> Current.ignore_value )
         :: ocaml_images
        @ archive_images)
    in
    ...
    let gen_tags _images full_tag switch distro_aliases =
      let tags =
        ...
        if switch <> Ocaml_version.Releases.latest then tags
          ...
          if distro <> master_distro then tags else Tag.latest :: tags
      in
      (* Fmt.pr "Aliases: %s -> %a@." full_tag Fmt.(Dump.list string) (List.sort String.compare tags); *)
      (Switch_map.empty, tags)
    in
    let _multiarch_images, pipeline =
      pipeline ~ocluster repos distro gen_tags
    in
    pipeline
  ...
  let windows_distro_pipeline ~ocluster repos distro_label distro_versions =
    let distro_pipeline multiarches distro =
      ...
      in
      let multiarch_images, pipeline =
        pipeline ~ocluster repos distro gen_tags
      in
      let multiarch_images =
        let update images = function
          | None -> Some images
          | Some images' -> Some (images @ images')
        in
        let fold switch images acc =
          Switch_map.update switch (update images) acc
        in
      ...
      (multiarch_images, pipeline)
    in
    let multiarch, pipelines =
      List.fold_left_map distro_pipeline Switch_map.empty distro_versions
    in
    let pushes =
      Switch_map.fold
        (fun switch images pushes ->
          let full_tag =
            Printf.sprintf "%s:%s-ocaml-%s" Conf.public_repo distro_label
              (Tag.tag_of_compiler switch)
          in
          let tags =
    ...
    in
    Current.(
      collapse ~key:"distro" ~value:distro_label ~input:repos
        (all (all_labelled pushes :: pipelines)))

  let windows_pipeline ~ocluster repos mingw msvc cygwin =
    List.filter_map
      (fun (distro_label, distros) ->
        match distros with
        | [] -> None
        | distro_versions ->
            let repos = label distro_label repos in
            windows_distro_pipeline ~ocluster repos distro_label distro_versions
            |> Option.some)
  ...
  let v ~ocluster repos =
    let linux, mingw, msvc, cygwin =
      Conf.distros
      |> List.fold_left
           (fun (linux, mingw, msvc, cygwin) distro ->
             let os_family = Distro.os_family_of_distro distro in
             match os_family with
    ...
    let pipelines =
      ...
      @ windows_pipeline ~ocluster repos mingw msvc cygwin
    in
...
let notify_status ?channel x =
  ...
  | Some channel ->
      let s =
        ...
        Fmt.str "docker-base-images status: %a"
...
let v ?channel ~ocluster () =
  if Conf.auth = None then
    Fmt.pr "Password file %S not found; images will not be pushed to hub@."
      Conf.password_path;
