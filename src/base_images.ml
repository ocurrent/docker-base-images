...
module Metrics = struct
  ...
  let family =
    ...
    Gauge.v_labels ~label_names:[ "platform"; "state" ] ~help ~namespace
      ~subsystem "image_state_total"
  ...
  let update () =
    ...
    let f opam_map platform sm =
      ...
      in
      let stats =
        Option.map (incr_stats stats)
          (Index.Platform_map.find_opt platform opam_map)
        |> Option.value ~default:stats
      in
      Gauge.set (Gauge.labels family [ platform; "ok" ]) (float_of_int stats.ok);
      Gauge.set
        (Gauge.labels family [ platform; "failed" ])
        (float_of_int stats.failed);
      Gauge.set
        (Gauge.labels family [ platform; "active" ])
...
let has_role user = function
  ...
  | _ -> (
      ...
      | Some
          ...
          | "github:samoht" | "github:dra27" | "github:mtelvers"
          | "github:tmcgilchrist" | "github:MisterDA" | "github:moyodiallo"
          | "github:benmandrew" ) ->
          true
      | _ -> false)

let main () config mode channel capnp_address github_auth submission_uri
    staging_password_file prometheus_config =
  Lwt_main.run
    (let channel = Option.map read_channel_uri channel in
     let staging_auth =
       staging_password_file
       |> Option.map (fun path -> (staging_user, read_first_line path))
     in
     ...
     let ocluster =
       Current_ocluster.v ?push_auth:staging_auth ~urgent:`Never connection
     in
     let engine =
       Current.Engine.create ~config (Pipeline.v ?channel ~ocluster)
     in
     rpc_engine_resolver
     |> Option.iter (fun r -> Capability.resolve_ok r (Rpc.engine engine));
     ...
     in
     let secure_cookies = channel <> None in
     (* TODO: find a better way to detect production use *)
     let routes =
       Routes.((s "login" /? nil) @--> Current_github.Auth.login github_auth)
       :: Current_web.routes engine
     in
     let site =
       Current_web.Site.v ?authn ~secure_cookies ~has_role ~name:program_name
         ~refresh_pipeline:60 routes
     in
     let prometheus =
...
let parse_tcp s =
  ...
  | Some (host, port) -> (
      ...
      | Some port -> Ok (Capnp_rpc_unix.Network.Location.tcp ~host ~port))
...
let staging_password =
  ...
  @@ Arg.info
       ~doc:
         (Printf.sprintf
            "A file containing the password for the %S Docker Hub user"
            staging_user)
       ~docv:"FILE"
