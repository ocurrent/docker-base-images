...
module Repositories = struct
      ...
      let json =
        `Assoc
          [
            ...
            ( "opam-repository-mingw__sunset",
              `String opam_repository_mingw_sunset );
  ...
  module Value = struct
    ...
    let unmarshal s =
      ...
      | Ppx_deriving_yojson_runtime.Result.Error _ ->
          failwith "failed to parse Git_repositories.Repositories.Value.t"
  end
  ...
  let get_commit_hash ~job ~repo ~branch =
    ...
    Current.Process.check_output ~cwd ~cancellable:true ~job
      ("", [| "git"; "rev-parse"; "HEAD" |])
    >>!= fun hash -> Lwt.return (Ok (String.trim hash))
  ...
  let build No_context job
    ...
    get_commit_hash ~job ~repo:opam_repository_master ~branch:"master"
    >>!= fun opam_repository_master ->
    get_commit_hash ~job ~repo:opam_repository_mingw_sunset ~branch:"sunset"
    >>!= fun opam_repository_mingw_sunset ->
    get_commit_hash ~job ~repo:opam_overlays ~branch:"overlay"
    >>!= fun opam_overlays ->
    get_commit_hash ~job ~repo:opam_2_0 ~branch:"2.0" >>!= fun opam_2_0 ->
    get_commit_hash ~job ~repo:opam_2_1 ~branch:"2.1" >>!= fun opam_2_1 ->
    get_commit_hash ~job ~repo:opam_master ~branch:"master"
    >>!= fun opam_master ->
    Lwt.return
      (Ok
         {
           Value.opam_repository_master;
           opam_repository_mingw_sunset;
           opam_overlays;
           opam_2_0;
           opam_2_1;
...
let get ~schedule =
  let key =
    {
      Repositories.Key.opam_repository_master =
  ...
  in
  let+ {
         Repositories.Value.opam_repository_master;
         opam_repository_mingw_sunset;
         opam_overlays;
         opam_2_0;
         opam_2_1;
  ...
  {
    opam_repository_master =
      Current_git.Commit_id.v ~repo:key.opam_repository_master ~gref:"master"
        ~hash:opam_repository_master;
    opam_repository_mingw_sunset =
      Current_git.Commit_id.v ~repo:key.opam_repository_mingw_sunset
        ~gref:"sunset" ~hash:opam_repository_mingw_sunset;
    opam_overlays =
      Current_git.Commit_id.v ~repo:key.opam_overlays ~gref:"overlay"
        ~hash:opam_overlays;
    opam_2_0 =
    ...
    opam_master =
      Current_git.Commit_id.v ~repo:key.opam_master ~gref:"master"
        ~hash:opam_master;
  }
